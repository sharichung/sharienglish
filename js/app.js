...
`;
  ...
  group.phonics.forEach(p => {
    ...
    const col = document.createElement("div");
    col.className = `${colClass} d-flex`; // Makes column a flex container
    ...
    waveBg.style.color = `var(--color-${color}-600)`; // 主題色
    waveBg.innerHTML = `
  ...
  <div class="wave-bar-bg"></div>
`;
    card.style.position = "relative"; // 確保背景定位正確
  ...
  `;
    ...
    const button = document.createElement("button");
    button.className = `btn w-100 d-flex align-items-center justify-content-center text-white fs-5 theme-${color}`;
    button.style.backgroundColor = `var(--color-${color}-500)`;
    button.onmouseenter = () => button.style.backgroundColor = `var(--color-${color}-600)`;
    button.onmouseleave = () => button.style.backgroundColor = `var(--color-${color}-500)`;
  ...
  `;
    button.onclick = () => {
      ...
      currentAudio.onended = () => {
        waveBg.style.opacity = "0"; // 或 waveBg.remove();
      };
    };
  ...
  function createDraggableLetter(letter) {
    ...
    span.addEventListener("click", () => {
      ...
      let phonics = null;
      for (let i = 1; i <= currentGroupId; i++) {
        const g = groups.find(g => g.id === i);
        if (!g) continue;
        const found = g.phonics.find(p => p.letter.toLowerCase() === letter.toLowerCase());
        if (found) phonics = found;
      }
